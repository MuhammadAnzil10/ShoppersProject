...
const addCategory = async(req,res)=>{
    ...
    try{
        ...
        if(req.body)
        {
           ...
           await categoryHelper.addCategory(req.body).then((data)=>{
...
const categoryEdit = async (req,res)=>{
    ...
    try{
        if(req.body)
        {
         ...
         categoryHelper.updateCategory(req.body).then((data)=>{
            res.redirect('/admin/category')
         }).catch((error)=>{
            res.render('categoryEdit',{category,message:'Category already existing'})
         })
...
const blockCategory = async (req,res)=>{
    ...
    try{
        ...
        const blockedCategory = await Category.findByIdAndUpdate({_id:categoryId},{$set:{unlist:true}})
...
const unblockCategory = async (req,res)=>
{
    try{
            ...
            const unblockedCategory = await Category.findByIdAndUpdate({_id:categoryId},{$set:{unlist:false}})
            // res.redirect('/admin/category')
